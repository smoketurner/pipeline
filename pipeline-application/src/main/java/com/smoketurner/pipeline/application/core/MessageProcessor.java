...
public class MessageProcessor implements Predicate<Message> {
    ...
    public MessageProcessor(@Nonnull final MetricRegistry registry,
        ...
        this.eventCounts = registry
                .histogram(name(MessageProcessor.class, "event-counts"));
    }
    ...
    public boolean test(@Nullable final Message message) {
        ...
        if (broadcaster.isEmpty()) {
            LOGGER.debug(
                    "No connections found, skipping SQS message processing");
            return false;
        }

        final AmazonSNSNotification notification;
        try {
            notification = MAPPER.readValue(message.getBody(),
                    AmazonSNSNotification.class);
        } catch (IOException e) {
            LOGGER.error(
                    "Failed to parse SNS notification, deleting SQS message",
                    e);
            return true;
        }
        ...
        LOGGER.debug("Parsed {} S3 event records from SNS notification",
                recordCount);

        if (recordCount < 1) {
            LOGGER.debug(
                    "No S3 event records found in SNS notification, deleting SQS message");
            return true;
        }
        ...
        if (recordsProcessed == recordCount) {
            LOGGER.debug("Processed {} of {} records, deleting SQS message",
