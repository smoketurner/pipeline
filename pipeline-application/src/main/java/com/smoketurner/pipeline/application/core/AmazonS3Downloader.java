...
public class AmazonS3Downloader {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(AmazonS3Downloader.class);
    ...
    public S3Object fetch(@Nonnull final S3EventNotificationRecord record)
        ...
        if (download == null) {
            LOGGER.error("eTag from object did not match for key: {}/{}",
                    object.getBucketName(), object.getKey());
            throw new AmazonS3ConstraintException(object.getKey());
        }

        final long contentLength = download.getObjectMetadata()
                .getContentLength();
        if (contentLength < 1) {
            try {
                download.close();
        ...
        LOGGER.debug("Streaming key ({} bytes): {}/{}", contentLength,
                download.getBucketName(), download.getKey());
    ...
    public static boolean isGZipped(@Nullable final S3Object object) {
        ...
        final String encoding = Strings
                .nullToEmpty(object.getObjectMetadata().getContentEncoding());
